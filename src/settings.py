...
# --- Core Paths ---
BASE_DIR = pathlib.Path(__file__).resolve().parent.parent  # Project Root (src/..)
BIN_DIR = BASE_DIR / "bin"  # For binaries, configs, and generated files
...
NGINX_CONFIG_PATH = BIN_DIR / "nginx.conf"
...
http {{
    ...
    # Serves pre-optimized media directly.
    server {{
        listen {listen_port};
        ...
        location / {{
            ...
            # Tries to serve the pre-converted file first (.avif, .webm).
            # If not found, falls back to the @asset_fallback location.
            try_files $uri @asset_fallback;
        }}
        
        # This named location is the fallback for assets. It proxies the request
        # to the Python app with a special query parameter, asking for the *original* file.
        location @asset_fallback {{
            # Add the query parameter to signal a request for the original file.
    ...
    # Proxies all traffic to the backend ASGI server.
    server {{
        listen {listen_port};
...
(SRC_DIR / "templates").mkdir(parents=True, exist_ok=True)
(ROOT_INDEX_DIR / ".assets").mkdir(exist_ok=True)
